...
3. 対象月の daily を集める。`daily/` を再帰的に見て、`untagged/` と `tagged/` の**両方から**集める。タグ付けが済んでいない日を落とさないため(完全性保証)。`_trash/` と `slices/` は見ない。`slices/` は daily の本文をそのまま持つ派生層なので、含めるとタグも記述も二重に数えることになる。
4. 各 daily の frontmatter からタグを数え、分布を出す。ここでは本文を読まない。未タグの枚数は `daily/untagged/` にある対象月のファイル数で数える。
...
対象枚数と、そのうち `daily/untagged/` に残っている枚数を必ず示す。未タグの daily を含めたことが読み手に見えるようにするため。
