...
2. `rg -n '^IDEA:|^Q:' daily` で該当行だけを集める。本文を広く読まない。
3. 新しい順、または近い話題ごとにまとめて提示する。各項目に出典 `[[daily/YYYY-MM-DD]]` を添える。
...
- 走査対象は `daily/` だけ。`slices/` は daily の行をそのまま持つ派生層なので、含めると同じ着想を2回拾う。
